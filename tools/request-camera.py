...
p.add_argument('--require-draw',action='store_true',help='Only sample sets with supported recorded draw associations')
a=p.parse_args()
...
if a.require_draw and not s.get('draw_associations_supported',False):p.error('Draw associations unavailable: '+s.get('draw_associations','restart with updated layer'))
try:os.kill(s['pid'],0)
...
request={'id':time.time_ns(),'device':device,'duration_ms':a.duration_ms,'max_samples':a.samples,'require_draw':a.require_draw}
