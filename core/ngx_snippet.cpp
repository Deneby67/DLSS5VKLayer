...
bool NgxLoadAndInit(NgxSnippet& s, VkInstance instance, VkPhysicalDevice pd, VkDevice device,
    ...
    if (!InstallCallerSpoof(s.snippet, g_snippetSpoof, g_layerModule)) { s.disabled = true; return false; }
    ...
    if (FileExists(corePath)) {
        ...
        if (s.core) InstallCallerSpoof(s.core, g_coreSpoof, g_layerModule);
